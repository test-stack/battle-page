                ...
                <div className="btn-group mr-2" role="group" aria-label="First group">
                  {
                    ...
                    todos.map((todo, i) => {
                      ...
                      if (_todoCounter === paginationSize) {
                        ...
                        return (
                          <button type="button"
                                  className={ `btn btn-${_pages === this.state.page ? 'primary' : 'secondary'}` }
                                  key={_pages} id={ `page-${_pages}` }
                                  onClick={this.setPage.bind(this, _pages)}>{_pages}</button>
                        )
                      }
                    })
                  }
                  ...
                  <button type="button"
                          className={ `btn btn-${Math.ceil(todos.length / paginationSize) === this.state.page ? 'primary' : 'secondary'}` }
                          key={Math.ceil(todos.length / paginationSize)} id={ `page-${_pages}` }
                          onClick={() => this.setPage(Math.ceil(todos.length / paginationSize))}>{Math.ceil(todos.length / paginationSize)}</button>
                  }
                </div>
              </div>
              }
          ...
          <div className="col-md-4" id="pageRightBlock">
            <div className="card">
              <div className="card-block">
                ...
                <p className="lead">Zobrazuji <strong><span
                  id="paginationSize">{ pagination ? paginationSize : todos.length }</span></strong>{' '}
                ...
                <div className="btn-group btn-group-sm" role="group" aria-label="Basic example">
                  <button type="button" id="activatePagination"
                          className={ `btn btn-${pagination ? 'success' : 'secondary'}` }
                          onClick={() => this.props.togglePagination(true)}>Zapnout
                  </button>
                  <button type="button" id="deactivatePagination"
                          className={ `btn btn-${pagination ? 'secondary' : 'success'}` }
                          onClick={() => this.props.togglePagination(false)}>Vypnout
                  </button>
                </div>
                { pagination &&
                <div className="form-group">
                  ...
                  <input type="text" className="form-control form-control-sm" id="sizePagination"
                         defaultValue={paginationSize} onChange={() => this.props.setPaginationSize()}/>
                </div>
                }
                { todos.length > 1 &&
                <div>
                  ...
                  <div className="btn-group btn-group-sm" role="group" aria-label="Basic example">
                    <button type="button" id="showTodosDeleteModalButton" className="btn btn-secondary"
                            onClick={() => this.props.toggleDeleteModal()}>Smazat
                    </button>
                  </div>
                </div>
                }
